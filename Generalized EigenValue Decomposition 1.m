% HW3 - 97200216
clc; clear all; close all;
%%
load('HW3_Ex1_data.mat')
load('HW3_Ex1_Sources.mat')
fs = 100;
t = (0:10000-1)/fs;
% plot(t,X_org)
N = length(X_org);
%% Triangle GEVD
Cx_org = cov(X_org'); 
a = zeros(8,8);
for i=1:N-4*fs
    a = a + (X_org(:,i)-mean(X_org,2))*transpose(X_org(:,i+4*fs)-mean(X_org,2));
end

Px_org = a ./ (N-4*fs);
Px_tilda = 0.5.*(Px_org + Px_org');
[V,D] = eig(Px_tilda,Cx_org);
w1 = V(:,8);
S = V' * X_org;
S_denoised = S;
S_denoised(1:7,:) = 0;
X1_hat = inv(V') * S_denoised + repmat(mean(X_org,2),1,N);
error_X1 = (sum(sum((X1-X1_hat).^2))/sum(sum(X1.^2)))^0.5;
for i=1:8
    figure(1)
    subplot(4,2,i)
    plot(t,X1_hat(i,:),t,X1(i,:))
    title(['channel',num2str(i)])
    legend('X1-hat','X1')
end
    
%%
for i=3*fs:7*fs
    cor1(i-3*fs+1) = corr2(X_org(1,1:end-i),X_org(1,i+1:end));
    cor2(i-3*fs+1) = corr2(X_org(2,1:end-i),X_org(2,i+1:end));
    cor3(i-3*fs+1) = corr2(X_org(3,1:end-i),X_org(3,i+1:end));
    cor4(i-3*fs+1) = corr2(X_org(4,1:end-i),X_org(4,i+1:end));
    cor5(i-3*fs+1) = corr2(X_org(5,1:end-i),X_org(5,i+1:end));
    cor6(i-3*fs+1) = corr2(X_org(6,1:end-i),X_org(6,i+1:end));
    cor7(i-3*fs+1) = corr2(X_org(7,1:end-i),X_org(7,i+1:end));
    cor8(i-3*fs+1) = corr2(X_org(8,1:end-i),X_org(8,i+1:end));  
end
Corr =  cor1 + cor2 + cor3 + cor4 +  cor5 + cor6 + cor7 + cor8; 
[M(1),I(1)] = max(cor1);
[M(2),I(2)] = max(cor2);
[M(3),I(3)] = max(cor3);
[M(4),I(4)] = max(cor4);
[M(5),I(5)] = max(cor5);
[M(6),I(6)] = max(cor6);
[M(7),I(7)] = max(cor7);
[M(8),I(8)] = max(cor8);
[s,i1] = sort(M);
p = (I(i1(8))+I(i1(7))+I(i1(6)))/3;
T_period = round((p+3*fs-1))/fs;
N_T = round(T_period*fs);
a = zeros(8,8);
for i=1:N-N_T
    a = a + (X_org(:,i)-mean(X_org,2))*transpose(X_org(:,i+N_T)-mean(X_org,2));
end
% for i=1:N-N_T
%     a = a + (X_org(:,i))*transpose(X_org(:,i+N_T));
% end
Px_org_T = a ./ (N-N_T);
Px_tilda_T = 0.5.*(Px_org_T + Px_org_T');
[V_T,D_T] = eig(Px_tilda_T,Cx_org);
S_T = V_T' * X_org;
S_denoised_T = S_T;
S_denoised_T(1:7,:) = 0;
X1_hat_T = inv(V_T') * S_denoised_T + repmat(mean(X_org,2),1,N);
error_X1_T = (sum(sum((X1-X1_hat_T).^2))/sum(sum(X1.^2)))^0.5;
for i=1:8
    figure(2)
    subplot(4,2,i)
    plot(t,X1_hat_T(i,:),t,X1(i,:))
    title(['channel',num2str(i)])
    legend('X1-hat','X1')
end

%% On-Off

a = zeros(8,8);
m = 0;
for i=1:N
    if(T1(i)==1)
        a = a + (X_org(:,i)-mean(X_org,2))*transpose(X_org(:,i)-mean(X_org,2));
        m = m+1;
    end
end
Px_on = a ./ m;
[V_on,D_on] = eig(Px_on,Cx_org);
S_on = V_on' * X_org;
S_denoised_on = S_on;
S_denoised_on(1:7,:) = 0;
X2_hat = inv(V_on') * S_denoised_on + repmat(mean(X_org,2),1,N);
error_X2 = (sum(sum((X2-X2_hat).^2))/sum(sum(X2.^2)))^0.5;
for i=1:8
    figure(3)
    subplot(4,2,i)
    plot(t,X2_hat(i,:),t,X2(i,:))
    title(['channel',num2str(i)])
    legend('X2-hat','X2')
end
%%
% b1 = find(T2 == 1);
% Px_on1 = cov(transpose(X_org(:,b1)));
a = zeros(8,8);
m = 0;
for i=1:N
    if(T2(i)==1)
        a = a + (X_org(:,i)-mean(X_org,2))*transpose(X_org(:,i)-mean(X_org,2));
        m = m+1;
    end
end
Px_on1 = a ./ m;
[V_on1,D_on1] = eig(Px_on1,Cx_org);
S_on1 = V_on1' * X_org;
S_denoised_on1 = S_on1;
S_denoised_on1(1:7,:) = 0;
X2_hat1 = inv(V_on1') * S_denoised_on1 + repmat(mean(X_org,2),1,N);
error_X2_T2 = (sum(sum((X2-X2_hat1).^2))/sum(sum(X2.^2)))^0.5;
for i=1:8
    figure(4)
    subplot(4,2,i)
    plot(t,X2_hat(i,:),t,X2(i,:))
    title(['channel',num2str(i)])
    legend('X2-hat-T2','X2')
end

%% Fequency
Y = fft((X_org-repmat(mean(X_org,2),1,N))');
Y = abs(Y');
f = fs*(0:(N-1))/N;

a = zeros(8,8);
m = 0;
a = zeros(8,8);
m = 0;
for i=2:N
        a = a + (Y(:,i)-mean(Y,2))*transpose(Y(:,i)-mean(Y,2));
        m = m+1;
end
Cx_FFT = real(a./m);

a1 = zeros(8,8);
m1 = 0;
for i=1:N
    if(f(i)>=10 && f(i)<=15)
        a1 = a1 + (Y(:,i)-mean(Y,2))*transpose(Y(:,i)-mean(Y,2));
        m1 = m1+1;
    end
    if(f(i)>=85 && f(i)<=90)
        a1 = a1 + (Y(:,i)-mean(Y,2))*transpose(Y(:,i)-mean(Y,2));
        m1 = m1+1;
    end
end
Px_FFT = real(a1 ./ m1);
[V_FFT,D_FFT] = eig(Px_FFT,Cx_FFT,'chol');
S_FFT = V_FFT' * X_org;
S_denoised_FFT = S_FFT;
S_denoised_FFT(1:7,:) = 0;
X3_hat = inv(V_FFT') * S_denoised_FFT + repmat(mean(X_org,2),1,N);
error_X3 = (sum(sum((X3-X3_hat).^2))/sum(sum(X3.^2)))^0.5;

Y3_hat = (fft(X3_hat'));
Y3_hat = Y3_hat';
Y3_hat = abs(Y3_hat);
Y3 = (fft(X3'));
Y3 = Y3';
Y3 = abs(Y3);

f = fs*(0:(N-1))/N;
for i=1:8
    figure(5)
    subplot(4,2,i)
    plot(f,Y3_hat(i,:))
    title(['channel',num2str(i)])
%     legend('X3-hat','X3')
end


%%
Y = fft((X_org-repmat(mean(X_org,2),1,N))');
Y = abs(Y');
f = fs*(0:(N-1))/N;

a = zeros(8,8);
m = 0;
for i=1:N
        a = a + (Y(:,i)-mean(Y,2))*transpose(Y(:,i)-mean(Y,2));
        m = m+1;
end
Cx_FFT1 = a./m;

for i=1:100
    for j=1:8
        BP(j,i) = bandpower(X_org(j,:)-repmat(mean(X_org(j,:)),1,N),fs,[5+(i-1)*0.2 5+i*0.2]);
    end
    
end
% for i=1:8
%     figure(10)
%     subplot(4,2,i)
%     plot(BP(i,:))
% end

f_on = zeros(100,1);
for i=1:8
    a = find(abs(BP(i,:))>(10^-3));
    f_on(a) = 1;
end
ff = find(f_on==1);
f_min = 5 + ff(1)*0.2;
f_max = 5 + ff(end)*0.2;

a1 = zeros(8,8);
m1 = 0;
for i=1:N
    if(f(i)>=f_min && f(i)<=f_max)
        a1 = a1 + (Y(:,i)-mean(Y,2))*transpose(Y(:,i)-mean(Y,2));
        m1 = m1+1;
    end
    if(f(i)>=(100-f_max) && f(i)<=(100-f_min))
        a1 = a1 + (Y(:,i))*transpose(Y(:,i));
        m1 = m1+1;
    end
end
Px_FFT1 = a1 ./ m1;

[V_FFT1,D_FFT1] = eig(Px_FFT1,Cx_FFT1);
S_FFT1 = V_FFT1' * X_org;
S_denoised_FFT1 = S_FFT1;
S_denoised_FFT1(1:7,:) = 0;
X3_hat1 = inv(V_FFT1') * S_denoised_FFT1 + repmat(mean(X_org,2),1,N);
error_X3_1 = (sum(sum((X3-X3_hat1).^2))/sum(sum(X3.^2)))^0.5;

Y3_hat = (fft(X3_hat1'));
Y3_hat = Y3_hat';
Y3_hat = abs(Y3_hat);
Y3 = (fft(X3'));
Y3 = Y3';
Y3 = abs(Y3);

f = fs*(0:(N-1))/N;
for i=1:8
    figure(6)
    subplot(4,2,i)
    plot(f,Y3_hat(i,:))
    title(['channel',num2str(i)])
%     legend('X3-hat','X3')
end

%%
Y = fft((X_org-repmat(mean(X_org,2),1,N))');
Y = abs(Y');
f = fs*(0:(N-1))/N;

a = zeros(8,8);
m = 0;
for i=1:N
        a = a + (Y(:,i)-mean(Y,2))*transpose(Y(:,i)-mean(Y,2));
        m = m+1;
end
Cx_FFT1 = a./m;

a1 = zeros(8,8);
m1 = 0;
for i=1:N
    if(f(i)>=5 && f(i)<=25)
        a1 = a1 + (Y(:,i)-mean(Y,2))*transpose(Y(:,i)-mean(Y,2));
        m1 = m1+1;
    end
    if(f(i)>=75 && f(i)<=95)
        a1 = a1 + (Y(:,i))*transpose(Y(:,i));
        m1 = m1+1;
    end
end
Px_FFT1 = a1 ./ m1;

[V_FFT1,D_FFT1] = eig(Px_FFT1,Cx_FFT1);
S_FFT1 = V_FFT1' * X_org;
S_denoised_FFT1 = S_FFT1;
S_denoised_FFT1(1:7,:) = 0;
X3_hat1 = inv(V_FFT1') * S_denoised_FFT1 + repmat(mean(X_org,2),1,N);
error_X3_2 = (sum(sum((X3-X3_hat1).^2))/sum(sum(X3.^2)))^0.5;

Y3_hat = (fft(X3_hat1'));
Y3_hat = Y3_hat';
Y3_hat = abs(Y3_hat);
Y3 = (fft(X3'));
Y3 = Y3';
Y3 = abs(Y3);

f = fs*(0:(N-1))/N;
for i=1:8
    figure(7)
    subplot(4,2,i)
    plot(f,Y3_hat(i,:))
    title(['channel',num2str(i)])
%     legend('X3-hat','X3')
end